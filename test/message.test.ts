...
describe('message helpers', () => {
  it('appends the detailed report link using the required two-line format', () => {
    const text = buildDailyMessage('今天两融情绪偏热。\n融资净买入明显走强。\n关注后续持续性。', 'https://cos.example/report.html', 'GPT 5.4 (xhigh)');
  ...
  it('normalizes single-paragraph text into multiple lines', () => {
    const text = buildDailyMessage('今天两融情绪偏热。融资净买入明显走强。关注后续持续性。', 'https://cos.example/report.html', 'GPT 5.4 (xhigh)');
    expect(text).toContain('今天两融情绪偏热。\n融资净买入明显走强。\n关注后续持续性。');
  });

  it('formats alert, heartbeat, and failure messages', () => {
    expect(buildAlertMessage('overheat', '融资余额分位过高。5日融资净买入分位过高。', 'https://cos.example/report.html', 'GPT 5.4 (xhigh)')).toContain('🤖 模型：GPT 5.4 (xhigh)');
    expect(buildAlertMessage('overheat', '融资余额分位过高。5日融资净买入分位过高。', 'https://cos.example/report.html', 'GPT 5.4 (xhigh)')).toContain('详细版报告:');
    expect(buildAlertMessage('overheat', '融资余额分位过高。5日融资净买入分位过高。', 'https://cos.example/report.html', 'GPT 5.4 (xhigh)')).toContain('融资余额分位过高。\n5日融资净买入分位过高。');
