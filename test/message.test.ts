...
describe('message helpers', () => {
  ...
  it('normalizes single-paragraph text into multiple lines', () => {
    const text = buildDailyMessage('今天两融情绪偏热。融资净买入明显走强。关注后续持续性。', 'https://cos.example/report.md');
    expect(text).toContain('今天两融情绪偏热。\n融资净买入明显走强。\n关注后续持续性。');
  });

  it('formats alert, heartbeat, and failure messages', () => {
    expect(buildAlertMessage('overheat', '融资余额分位过高。5日融资净买入分位过高。', 'https://cos.example/report.md')).toContain('详细版报告:');
    expect(buildAlertMessage('overheat', '融资余额分位过高。5日融资净买入分位过高。', 'https://cos.example/report.md')).toContain('融资余额分位过高。\n5日融资净买入分位过高。');
