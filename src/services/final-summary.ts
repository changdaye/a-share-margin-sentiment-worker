...
async function summarizeWithLLM(config: AppConfig, ai: Ai | undefined, messages: string[]): Promise<{ content: string; modelLabel: string }> {
...
export async function runFinalSummary(env: Env, config: AppConfig, now = new Date()): Promise<FinalSummaryResult | null> {
  const prefix = 'a-share-margin-sentiment-worker/feishu-messages/';
