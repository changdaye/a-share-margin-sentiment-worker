...
- KV 保存运行时状态
- 若 D1 为空或历史不足，运行时会自动补最近一段历史窗口后再继续生成日报
...
4. A 股成交量补充使用腾讯公开 A 股指数日线（上证A股指数 + 深证A指）求和

## 历史数据策略

- **D1 是主存储**：历史两融快照、成交量补充字段、历史信号都会维护在 D1
- **支持手动回填**：可一次性补齐历史数据
- **支持冷启动自愈**：若服务迁移导致 D1 为空，任务运行时会自动补最近计算窗口所需历史，再继续出日报
...
```bash
npm run backfill:history:local
```
...
```bash
npm run backfill:recent:local
npm run backfill:recent:remote
```
